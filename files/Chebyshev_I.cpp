...
int Chebyshev_I::LP_Init(double Fs, double Fn, double g_stop, double F_stop, int order, bool mode_g_pu, bool mode_F_pu)
{
...
int Chebyshev_I::HP_Init(double Fs, double Fn, double g_stop, double F_stop, int order, bool mode_g_pu, bool mode_F_pu)
{
	// Fs     - частота дискретизации
	// Fn     - номинальная частота
	// g_stop - неравномерность в полосе подавления  ( если mode_g == true, то в о.е, если mode_g == false, то в Дб )
...
int Chebyshev_I::BP_Init(double Fs, double Fn, double g_stop, double F_stop1, double F_stop2, int order, bool mode_g_pu, bool mode_F_pu)
{
	// Fs     - частота дискретизации
	// Fn     - номинальная частота
	// g_stop - неравномерность в полосе подавления  ( если mode_g == true, то в о.е, если mode_g == false, то в Дб )
	...
	m_BUFF_GAIN_POLES.BuffInit(2 * m_L + m_R);  // буффер под коэффициенты усиления полиномов знаменателя после билинейного преобразования
	m_BUFF_GAIN_ZEROS.BuffInit(2 * m_L + m_R); // буффер под коэффициенты усиления полиномов числителя после билинейного преобразования

	//
	m_BUFF_BP_POLES_RE.BuffInit(2 * m_L + m_R);
	m_BUFF_BP_POLES_IM.BuffInit(2 * m_L + m_R);
	m_BUFF_BP_ZEROS_RE.BuffInit(2 * m_L + m_R);
	m_BUFF_BP_ZEROS_IM.BuffInit(2 * m_L + m_R);
...
int Chebyshev_I::BS_Init(double Fs, double Fn, double g_stop, double F_stop1, double F_stop2, int order, bool mode_g_pu, bool mode_F_pu)
{
	// Fs     - частота дискретизации
	// Fn     - номинальная частота
	// g_stop - неравномерность в полосе подавления  ( если mode_g == true, то в о.е, если mode_g == false, то в Дб )
...
int Chebyshev_I::FreqCharacteristics()
{
...
int Chebyshev_I::zeros_calc()
{
	return 0;
}
//------------------------------------------------------------------------------
// функция расчета полюсов нормированного фильтра Чебышева I рода:
int Chebyshev_I::poles_calc()
{
	// Считаем полюса фильтра:
	m_betta = asinh(1/m_epsilon_stop) / m_order; // вспомогательный коэффициент betta
	...
	if (m_R == 1)
	{
		for (int n = 0; n < m_L; n++)
		{
				...
				m_BUFF_POLES_RE.m_buff[n] = -sin(m_alpha)*sinh(m_betta);
				m_BUFF_POLES_IM.m_buff[n] = +cos(m_alpha)*cosh(m_betta);
		}

		// расчет некратного вещественного полюса с помещением в конец буффера:
		m_BUFF_POLES_RE.m_buff[m_L + m_R - 1] = -sinh(m_betta);
...
int Chebyshev_I::LP_to_LP_bilinear_transform()
{
...
int Chebyshev_I::LP_to_HP_bilinear_transform()
{
...
int Chebyshev_I::LP_to_BP_bilinear_transform()
{
...
int Chebyshev_I::LP_to_BS_bilinear_transform()
{
...
int Chebyshev_I::LPF_HPF_CoeffCalc()
{
	for (int n = 0; n < m_L; n++)
	{
...
int Chebyshev_I::BPF_BSF_CoeffCalc()
{
	if (m_filt_type == 3) // считаем полосовой фильтр
	{
		for (int n = 0; n < 2 * m_L; n++)
		{
			// Расчет коэффициентов квадратичных полиномов числителя ПФ:
	...
	if (m_filt_type == 4) // считаем заграждающий фильтр
	{
		for (int n = 0; n < 2 * m_L; n++)
		{
			// Расчет коэффициентов квадратичных полиномов числителя ПФ:
...
//------------------------------------------------------------------------------
// Функция фильтрации:
int Chebyshev_I::filt(double *in)
{
...
void Chebyshev_I::showFiltSpec()
{
	if (m_filt_type == 1 || m_filt_type == 2) 
	{
		std::cout << "Chebyshev I filter specification:" << "\n" << "\n";
...
void Chebyshev_I::showFiltZerosPoles()
{

	if ( m_filt_type == 1 || m_filt_type == 2 ) 
	{
		std::cout << "Chebyshev I filter zeros:" << "\n" << "\n";
...
void Chebyshev_I::showFiltCoeffs()
{
	std::cout << "Chebyshev I filter coeffs:" << "\n" << "\n";

	std::cout << "Second order sections gains:" << "\n" << "\n";
	m_BUFF_GAIN.ShowBuff();
	std::cout << "OutPutGain = " << m_output_Gain << "\n";
	std::cout << "\n" << "\n";
