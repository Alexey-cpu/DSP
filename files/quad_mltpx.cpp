...
quad_mltpx::quad_mltpx() 
{
	...
	m_FIR_dF.LP_Init ( 1000 / 5 , m_Fn , 5 , 10 * (m_Fs / m_HBuffSize / 250) , true); // порядок фильтра скольжения пересчитывается в зависимости от текущей длительности такта отностиельно такта длинной в  4 мс
...
int quad_mltpx::procInit(double Fs, double Fn, double SideLobeAtten , int FiltWindowWidth , int CycleWidth)
{
	...
	// инициализация фильтра скольжения (настройки по умолчанию):
	m_FIR_dF.LP_Init ( 1000 / CycleWidth , m_Fn , 5 , 10 * (m_Fs / m_HBuffSize / 250) , true); // порядок фильтра скольжения пересчитывается в зависимости от текущей длительности такта относительно такта длинной в 4 мс
	m_FIR_dF.m_WIND_FCN.Chebyshev(60);
