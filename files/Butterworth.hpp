...
#ifndef _BUTTERWORTH_HPP
#define _BUTTERWORTH_HPP
...
protected:
	...
	double m_output_Gain;// выходной коэффициент усиления
...
public:
	...
	int LP_Init(double Fs , double Fn , double g_stop , double F_stop  , int order , bool mode_g_pu , bool mode_F_pu);                     // функция инициализации инверсного ФНЧ Чебышева
	int HP_Init(double Fs , double Fn , double g_stop , double F_stop  , int order , bool mode_g_pu , bool mode_F_pu);                    // функция инициализации инверсного ФВЧ Чебышева
	int BP_Init(double Fs , double Fn , double g_stop , double F_stop1 , double F_stop2 , int order , bool mode_g_pu , bool mode_F_pu);  // функция инициализации инверсного ПФ Чебышева
	int BS_Init(double Fs , double Fn , double g_stop , double F_stop1 , double F_stop2 , int order , bool mode_g_pu , bool mode_F_pu); // функция инициализации инверсного РФ Чебышева
	...
	// функция отображения нулей и поюсов фильтра (отладочная, для пользования этой функией надо закоментить кусок с удалением массивов с нулями и полюсами в функции allocate, см. строку 1021):
