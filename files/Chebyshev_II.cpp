...
// Расчет БИХ фильтра Чебышева второго рода состоит из следующих этапов:
...
int Chebyshev_II::LP_Init(double Fs, double Fn, double g_stop, double F_stop, int order, bool mode_g_pu, bool mode_F_pu)
{
...
int Chebyshev_II::HP_Init(double Fs, double Fn, double g_stop, double F_stop, int order, bool mode_g_pu, bool mode_F_pu)
{
	// Fs     - частота дискретизации
	// Fn     - номинальная частота
	// g_stop - неравномерность в полосе подавления  ( если mode_g == true, то в о.е, если mode_g == false, то в Дб )
...
int Chebyshev_II::BP_Init(double Fs, double Fn, double g_stop, double F_stop1, double F_stop2, int order, bool mode_g_pu, bool mode_F_pu)
{
	// Fs      - частота дискретизации
	// Fn      - номинальная частота
	// g_stop  - неравномерность в полосе подавления          ( если mode_g == true, то в о.е, если mode_g == false, то в Дб )
...
int Chebyshev_II::BS_Init(double Fs, double Fn, double g_stop, double F_stop1, double F_stop2, int order, bool mode_g_pu, bool mode_F_pu)
{
	// Fs      - частота дискретизации
	// Fn      - номинальная частота
	// g_stop  - неравномерность в полосе подавления           ( если mode_g == true, то в о.е, если mode_g == false, то в Дб )
	...
	// Задание порядка ПФ / РФ (порядок должен быть всегда четным числом !!!):
	m_order = order / 2;
...
int Chebyshev_II::FreqCharacteristics()
{
...
//------------------------------------------------------------------------------
// функция расчета нулей нормированного фильтра Чебышева II рода:
int Chebyshev_II::zeros_calc() 
{
	// Считаем нули фильтра:
	for (int n = 0; n < m_L; n++)
	{
		m_alpha = (2 * (n+1) - 1) * PI0 / (2 * m_order);
		m_BUFF_ZEROS_IM.m_buff[n] = 1 / cos(m_alpha);
...
//------------------------------------------------------------------------------
// функция расчета полюсов нормированного фильтра Чебышева II рода:
int Chebyshev_II::poles_calc()
{
	// Считаем полюса фильтра:
	m_betta = asinh(m_epsilon_stop) / m_order; // вспомогательный коэффициент betta
	...
	if (m_R == 1)
	{
		...
		{
				m_alpha = (2 * (n + 1) - 1) * PI0 / (2 * m_order);
				m_BUFF_POLES_RE.m_buff[n] = -(sin(m_alpha)*sinh(m_betta)) / (cos(m_alpha)*cos(m_alpha)*cosh(m_betta)*cosh(m_betta) + sin(m_alpha)*sin(m_alpha)*sinh(m_betta)*sinh(m_betta));
				m_BUFF_POLES_IM.m_buff[n] = +(cos(m_alpha)*cosh(m_betta)) / (cos(m_alpha)*cos(m_alpha)*cosh(m_betta)*cosh(m_betta) + sin(m_alpha)*sin(m_alpha)*sinh(m_betta)*sinh(m_betta));
		}

		// расчет некратного вещественного полюса с помещением в конец буффера:
		m_BUFF_POLES_RE.m_buff[m_L + m_R - 1] = -1 / sinh(m_betta);
...
int Chebyshev_II::gain_calc()
{
	...
	for (int n = 0 ; n < m_L ; n++)
	{
		...
		m_BUFF_GAIN.m_buff[n] = 1 / (num / den);
	}
...
int Chebyshev_II::LP_to_LP_bilinear_transform()
{
...
int Chebyshev_II::LP_to_HP_bilinear_transform()
{
	// пересчет нулей и полюсов (для квадратичных полиномов, см. ПЗ):
...
int Chebyshev_II::LP_to_BP_bilinear_transform()
{
...
int Chebyshev_II::LP_to_BS_bilinear_transform() 
{
...
int Chebyshev_II::LPF_HPF_CoeffCalc()
{
...
int Chebyshev_II::BPF_BSF_CoeffCalc()
{
	for (int n = 0; n < 2 * m_L; n++)
	{
		// Расчет коэффициентов квадратичных полиномов числителя ПФ:
...
//------------------------------------------------------------------------------
// Функция фильтрации:
int Chebyshev_II::filt(double *in)
{
...
void Chebyshev_II::showFiltSpec()
{
	if (m_filt_type == 1 || m_filt_type == 2) 
	{
		std::cout << "Chebyshev II filter specification:" << "\n" << "\n";
...
void Chebyshev_II::showFiltZerosPoles()
{

	if ( m_filt_type == 1 || m_filt_type == 2 ) 
	{
		std::cout << "Chebyshev II filter zeros:" << "\n" << "\n";
...
void Chebyshev_II::showFiltCoeffs()
{
	std::cout << "Chebyshev II filter coeffs:" << "\n" << "\n";
