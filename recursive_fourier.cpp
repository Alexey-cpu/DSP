...
recursive_fourier::recursive_fourier()
{
        ...
        m_Gain     = 1 / m_Ns;
        m_hnum     = 1;
        m_Ks	   = sin( PI2 * m_hnum * m_Gain );
        m_Kc  	   = cos( PI2 * m_hnum * m_Gain );
        m_in_F     = m_Fn;
...
int recursive_fourier::FreqCharacteristics()
{
  ...
  m_Km = sqrt( Re1 * Re1 + Im1 * Im1 ) / sqrt( Re2 * Re2 + Im2 * Im2 ) * m_Gain;
...
void recursive_fourier::allocate()
{
    ...
    m_in_F = (m_Fn + 5) * m_hnum;
    FreqCharacteristics();
    m_FILTER_EQ.filtInit( m_Fs , m_Fn , 5.0 * m_hnum , ( 1 - m_Km ) * 100 );
    m_FILTER_EQ.allocate();
    m_oddh = ( (int)m_hnum % 2 != 0 );
