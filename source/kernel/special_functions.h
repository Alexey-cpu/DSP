...
*/
template< typename __type >
__type __get_data__( __type* _Data, int _Size, int _Index )
{
...
double __lagrange_polynom_interpolation__( double* _X, double* _Y, double _Point, int _Order )
{
    // check
    if( _X == nullptr || _Order <= 0 )
        return 0.0;
...
__samples_range_interpolation__(T1 _Input, T2 _Output, T3 _Gain, int _M, int _N, int _K, int _Order)
{
    ...
    for( int i = _Order, k = 0 ; i < _M ; i++ )
    {
        ...
        for( int m = 0 ; m < _Order+1 ; m++ )
        {
            ...
            {
                _Output[k] = __lagrange_polynom_interpolation__( X, Y, ( X[m] + (double)j * dN ), _Order + 1 ) * _Gain;
            }
...
__samples_range_decimation__(__InputType _Input, __OutputType _Output, int _M, int _N, int _K)
{
    // check
...
*/
template<typename __type> void
__dft__( Complex<__type>* _Input, Complex<__type>* _Spectrum, int _N, int _Direct )
{
    // check
...
*/
template<typename __type> void
__fft0__( Complex<__type>* _Spectrum, int _N, int _Direct )
{
    // check
...
*/
template<typename __type> void
__fft1__( Complex<__type>* _Spectrum, int _N, int _Direct )
{
    // check
    if( _Spectrum == nullptr || _N <= 0 || !check_power_of_two(_N) )
        return;
